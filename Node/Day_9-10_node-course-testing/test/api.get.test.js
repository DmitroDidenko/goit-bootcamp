...
describe('initial test', function () {
    it('should true to be true', () => {expect(true).to.be.true});
...
describe('/GET users', () => {
    ...
    // });
    it('it should GET all the users', (done) => {
        request.get('/api/v1.0/users').end((err, res) => {
                expect(res).to.have.status(200);
                expect(res.body).to.be.a('array');
                expect(res.body.length).to.not.be.equal(0);
                done();
    ...
    it('it should return a right error for the wrong GET request', (done) => {
        request.get('/api/v1.0/user').end((err, res) => {
                if (err || !res.ok) {
...
describe('GET /users/:id', () => {
    it('it should return the user by id', (done) => {
        ...
        request.get('/api/v1.0/users/' + userId).end((err, res) => {
                expect(res).to.have.status(200);

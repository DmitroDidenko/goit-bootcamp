...
const getUserByEmail = (arr, email) => arr.filter((item, key) => item.email === email);
...
const getUsersByFriend = (arr, name) => arr.filter(item => item.friends.includes(name)).map(arr => arr.name);
...
const getAllSkills = arr => arr.map(item => item.skills).reduce((acc, el) => acc.concat(el),[]).sort().filter((name, index, newArr) => name !== newArr[index + 1]);
...
const getUserNamesSortedByFriendsCount = arr => arr.sort((item1, item2) => item1.friends.length - item2.friends.length).map(el => el.name);
